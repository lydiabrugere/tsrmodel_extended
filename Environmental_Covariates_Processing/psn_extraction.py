...
psn_layer = Path('/Users/lianfeng/Document/species_richness_sdm/data/processed/nation_wide/wc2.1_30s_bio_15.tif')
col_names = ['grid_id', 'psn']
psn_var = pd.DataFrame(columns=col_names)
psn_var['grid_id'] = geom.grid_id

for index in range(geom.shape[0]):
    grid_id = int(geom.grid_id[index])
    Index_label = psn_var[psn_var['grid_id'] == grid_id].index.tolist()[0]
    polygon_geom = wkb.loads(geom.grid_polygon_geom[index], hex=True)

    features = [polygon_geom]
    try:
        with rio.open(psn_layer) as src:
        ...
        if len(non_zero_pixel_count) != 0:
            psn_var.at[Index_label, 'psn'] = float(out_image.mean())
        else:
...
psn_var.to_sql(name='pergrid_psn', schema='observation', con=engine, if_exists='replace', index=False)
