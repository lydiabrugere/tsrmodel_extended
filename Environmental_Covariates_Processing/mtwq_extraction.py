...
mtwq_layer = Path('/Users/lianfeng/Document/species_richness_sdm/data/processed/nation_wide/wc2.1_30s_bio_10.tif')
col_names = ['grid_id', 'mtwq']
mtwq_var = pd.DataFrame(columns=col_names)
mtwq_var['grid_id'] = geom.grid_id

for index in range(geom.shape[0]):
    grid_id = int(geom.grid_id[index])
    Index_label = mtwq_var[mtwq_var['grid_id'] == grid_id].index.tolist()[0]
    polygon_geom = wkb.loads(geom.grid_polygon_geom[index], hex=True)

    features = [polygon_geom]
    try:
        with rio.open(mtwq_layer) as src:
        ...
        if len(non_zero_pixel_count) != 0:
            mtwq_var.at[Index_label, 'mtwq'] = float(out_image.mean())
        else:
...
mtwq_var.to_sql(name='pergrid_mtwq', schema='observation', con=engine, if_exists='replace', index=False)

update_geom = """
alter table predictor.pergrid_mtwq add column wkb_geometry geometry(Polygon,4269);
update predictor.pergrid_mtwq A SET wkb_geometry = B.grid_polygon_geom
