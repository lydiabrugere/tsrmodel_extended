...
art_layer = Path('/Users/lianfeng/Document/species_richness_sdm/data/processed/nation_wide/wc2.1_30s_bio_7.tif')
col_names = ['grid_id', 'art']
art_var = pd.DataFrame(columns=col_names)
art_var['grid_id'] = geom.grid_id

for index in range(geom.shape[0]):
    grid_id = int(geom.grid_id[index])
    Index_label = art_var[art_var['grid_id'] == grid_id].index.tolist()[0]
    polygon_geom = wkb.loads(geom.grid_polygon_geom[index], hex=True)

    features = [polygon_geom]
    try:
        with rio.open(art_layer) as src:
        ...
        if len(non_zero_pixel_count) != 0:
            art_var.at[Index_label, 'art'] = float(out_image.mean())
        else:
...
art_var.to_sql(name='pergrid_art', schema='observation', con=engine, if_exists='replace', index=False)
